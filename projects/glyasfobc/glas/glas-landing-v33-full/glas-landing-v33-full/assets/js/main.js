document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(contactForm){
    ...
    const buildMessage=(data)=>[
      'Hola, llegué desde la web de GL&AS y quisiera realizar una consulta.',
      '',
      `Nombre: ${data.nombre}`,
      data.empresa ? `Empresa: ${data.empresa}` : null,
      `Correo: ${data.email}`,
      data.telefono ? `Teléfono: ${data.telefono}` : null,
      data.area ? `Área de interés: ${data.area}` : null,
      '',
      'Consulta:',
      data.mensaje
    ...
    const whatsappButton=contactForm.querySelector('[data-send-whatsapp]');
    const emailButton=contactForm.querySelector('[data-send-email]');

    whatsappButton?.addEventListener('click',()=>{
      if(!validateForm()) return;
      const data=getFormData();
      const message=buildMessage(data);
      const url=`https://wa.me/595994257076?text=${encodeURIComponent(message)}`;
    ...
    emailButton?.addEventListener('click',()=>{
      if(!validateForm()) return;
      const data=getFormData();
      const subject=`Consulta desde la web de GL&AS${data.area ? ' - '+data.area : ''}`;
      const body=buildMessage(data);
      const mailto=`mailto:Edgardo.listello@glyasfobc.com?cc=${encodeURIComponent('administracion@glyasfobc.com')}&subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
...
// V33 — copiar teléfono y correos desde la sección de contacto.
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const activateCopiedState=(button,label)=>{
    const feedback=button.closest('.contact-info-content')?.querySelector('.copy-feedback');
  ...
  document.querySelectorAll('[data-copy-phone],[data-copy-value]').forEach(button=>{
    button.addEventListener('click',async()=>{
      const value=button.dataset.copyPhone || button.dataset.copyValue || '';
      if(!value) return;
      await copyText(value);
      activateCopiedState(button,button.dataset.copyLabel || 'Número copiado');
    });
  });
});
